...
// fēnix 6 Sapphire renderer.
// The fēnix 6 Sapphire is in Garmin's fenix6pro device family and uses
// Connect IQ API 3.4, so this branch does not use the API 4.2 battery
// complication subscription used by the Forerunner 955 build.
class Typeface955Renderer {
    // Fixed 260x260 fēnix 6 Sapphire geometry.
    ...
    function refreshBatteryNow() {
        // Refresh once whenever the face becomes visible. This catches the
        // latest value after charging without adding a timer or frequent polling.
        sampleBattery();
    }
